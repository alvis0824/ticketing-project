    ...
    public List<ProjectDTO> getCountedListOfProjectDTO(UserDTO manager) {
        List<ProjectDTO> projectList = findAll().stream()
                ...
                .map(projectDTO -> {
                    ...
                    int completeTaskCounts = (int) taskList.stream().filter(task -> task.getProject().equals(projectDTO) && task.getTaskStatus() == Status.COMPLETE).count();
                    int unfinishedTaskCounts = (int) taskList.stream().filter(task -> task.getProject().equals(projectDTO) && task.getTaskStatus() != Status.COMPLETE).count();
